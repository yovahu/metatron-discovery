...
export class WorkspaceComponent extends AbstractComponent implements OnInit, OnDestroy {
  ...
  public checkAllEvent() {
    ...
    // 필터 목록 조회
    const filteredList: Book[] = this._getDisplayItems();
  ...
  public selectTargetLocation(event: MouseEvent, item: Workspace | Folder, back?: boolean) {
    ...
    if (back) {   // 뒤로가기 버튼이라면
      ...
      if ((item as Workspace).publicType) {
        // 워크스페이스인 경우
  ...
  private _setWorkspaceData(loadNbServer: boolean, workspace?: Workspace,) {
    if (workspace) {
      if (workspace.active) {
        ...
        this.workspace = workspace;
        // 워크스페이스 이름
        this.workspaceName = (workspace.publicType === PublicType.PRIVATE && workspace.owner && workspace.owner.fullName) ? '| ' + workspace.owner.fullName : workspace.name;
        // 워크스페이스 설명
